 ...
 *     via `navigator.gpu.requestDevice()` and dispatch the `.wgsl` compute shaders.
 *   - On WebGL2-only environments we fall back to a pure-JS implementation.
 *
 * TDR mitigation:
 *   The `meshBatch()` helper splits chunks across multiple animation frames
 *   when the estimated workload exceeds `TDR_BUDGET_SECONDS` per frame.
 */
    ...
    for (let z = 0; z < size; z++)
      for (let y = 0; y < size; y++)
        for (let x = 0; x < size; x++) {
          ...
          if (v !== 0) { solid[idx] = 1; mat[idx] = v & 0xff; }
        }
